...
def main():
    ...
    assert len(qc)==48 and len(records)==len({(int(r[0]),int(r[1])) for r in records})
    ...
    for hi in np.linspace(0,len(MANIFEST)-1,12,dtype=int):
        h=MANIFEST[hi]['hap']
    ...
    validation={'status':'PASS','source_step3_complete_sha256':source_sha,'source_step3_unchanged':sha(S3/'COMPLETE.json')==source_sha,'integrated_exact_candidates':len(exact),'mismatch_queries':len(queries),'new_query_radius2_neighborhoods_exhaustive_and_deduplicated':True,'variant_spectrum_serialization_audit_haps':12,'all_574_new_query_counts_match_step1_at_0mm':True,'all_valid_windows_match_step0':True,'mismatch_position_accounting_verified':True,'orientation_fields_verified':True,'plots_verified':len(plots)}
    (S4/'validation_summary.json').write_text(json.dumps(validation,indent=2)+'\n')
    summary={'status':'complete','integrated_exact_candidates':len(exact),'mismatch_evaluated_candidates':len(queries),'sequence_sets':574,'nonreference_haps':573,'strict_pan_pass_counts_by_radius':mm['strict_pan_pass_counts_by_radius'],'category_pass_counts_by_radius':mm['category_pass_counts_by_radius'],'robustness_tiers':mm['robustness_tiers'],'scope':'HSat2/HSat3 target discovery using existing annotations on eligible T2T chromosomes','interpretation':'Ranked targets for experimental ASO design; sequence-only quality proxies and genomic occurrence are not knockdown efficacy.'}
