...
ids<-c('k16_3e476af8','k16_a0d0562c','k16_4118ba0e');chr<-c(paste0('chr',1:22),'chrX','chrY')
...
for(kid in ids){
 d<-hits[hits$kmer_id==kid,];d$bin100kb<-floor(d$start_0based/1e5);aggregate<-aggregate(list(exact_hits=d$start_0based),list(chromosome=d$chromosome,bin100kb=d$bin100kb,outside=d$outside_broad_censat),length)
...
# Dedicated chr9 view with annotation tracks below the hit curves.
chr9<-hits[hits$chromosome=='chr9',];bc<-broad[broad$V1=='chr9',];ff<-fine[fine$V1=='chr9' & fine$V4=='HSat3',]
png(file.path(out,'chr9_censat_and_hits_separate_tracks.png'),width=2800,height=2100,res=180)
layout(matrix(1:4,ncol=1),heights=c(1,1,1,.5));par(oma=c(3,0,3,0));pal<-c('#C3532C','#247BA0','#7060A5')
for(i in seq_along(ids)){
 d<-chr9[chr9$kmer_id==ids[i],];bn<-floor(d$start_0based/1e5);tab100<-table(bn);x<-seq(480,770);y<-numeric(length(x));ix<-match(as.integer(names(tab100)),x);y[ix[!is.na(ix)]]<-as.numeric(tab100)[!is.na(ix)]
 par(mar=c(3,5,3,2));plot(x/10,y,type='l',xlim=c(48,77),lwd=2,col=pal[i],xlab='',ylab='Hits / 100 kb',main=sprintf('%s | inside CenSat %s | outside 0',ids[i],format(nrow(d),big.mark=',')));grid(col='gray92')
}
par(mar=c(5,5,2,2));plot(NA,xlim=c(48,77),ylim=c(0,2),yaxt='n',xlab='CHM13 chr9 position (Mb)',ylab='',main='Annotation intervals (shown separately from hits)')
...
mtext('CHM13 chr9 | exact-hit curves and CenSat annotation on separate tracks',outer=TRUE,side=3,line=1,font=2,cex=1.2)
mtext('Broad CenSat boundary: 39.95–81.69 Mb. HSat3 intervals are fine family annotations; all three 16-mers have zero exact hits outside CenSat.',outer=TRUE,side=1,line=1,cex=.75);dev.off()
