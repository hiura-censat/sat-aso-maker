...
def main():
 ...
 totals=read(OUT/'chm13_censat_totals.tsv');detail=read(OUT/'chm13_censat_by_chromosome.tsv');candidates=read(OUT/'chm13_candidate_censat_summary.tsv');gc=read(OUT/'group_chromosome_summary.tsv')
 ...
 assert all(r['chromosome'] in {f'chr{i}' for i in range(1,23)}|{'chrX','chrY'} and r['group'].startswith('G') for r in gc)
 ...
 index=['# 72 k-mer landscape figures','', 'CenSat outside exact-hit数が多い順。個別図にはCHM13座標、annotation、染色体別に独立して定義したgroupとcountを収録。拡大図はCHM13で最もヒットが多い染色体。','', '| k-mer | family | 用途 | CHM13 exact | CenSat外 | 個別図 | 拡大図 |','|---|---|---|---:|---:|---|---|']
 for r in candidates:
