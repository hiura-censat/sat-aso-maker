...
int main(int argc,char**argv){try{
 if(argc!=5)throw std::runtime_error("usage: step2_count codes.u32 chromosomes.txt counts.u64 stats.tsv < regions.fa");
 ...
 while(std::getline(std::cin,line)){
  ...
  for(unsigned char ch:line){if(ch=='\r')continue;int b=-1;switch(ch){case 'A':case 'a':b=0;break;case 'C':case 'c':b=1;break;case 'G':case 'g':b=2;break;case 'T':case 't':b=3;break;}
   ...
   valid[chr]++;auto it=index.find(std::min(fw,rc));if(it!=index.end())counts[(size_t(it->second)*nc+chr)*2+(fw>rc)]++;
  }
 }
