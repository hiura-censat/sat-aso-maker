...
int main(int argc,char**argv){try{
 ...
 if(mode=="pack"&&argc==3){gzFile out=gzopen(argv[2],"wb1");if(!out)throw runtime_error("open failed");string s;uint64_t n,total=0,distinct=0;while(cin>>s>>n){if(n){put(out,encode(s),n);total+=n;distinct++;}}if(!cin.eof())throw runtime_error("bad dump");if(gzclose(out)!=Z_OK)throw runtime_error("close failed");cout<<distinct<<"\t"<<total<<endl;}
 else if(mode=="pool"&&argc==6){
  uint64_t pooled_min=stoull(argv[4]),hap_min=stoull(argv[5]);
  if(!pooled_min||!hap_min)throw runtime_error("count minima must be positive");
  ...
  vector<uint32_t> keys;for(auto &p:all)if(p.second.sum>=pooled_min||p.second.maximum>=hap_min)keys.push_back(p.first);sort(keys.begin(),keys.end());
  ofstream out(argv[3],ios::binary);for(auto k:keys){out.write((char*)&k,4);}if(!out)throw runtime_error("write failed");
  cout<<"{\"distinct_target_kmers\":"<<all.size()<<",\"preliminary_candidates\":"<<keys.size()<<",\"pooled_min_count\":"<<pooled_min<<",\"hap_min_count\":"<<hap_min<<"}"<<endl;
 }else throw runtime_error("usage: pack out.bin.gz OR pool input-list output.u32 pooled-min hap-min");
