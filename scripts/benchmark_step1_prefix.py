...
def run(use):
 ...
 if use:cmd[0]=str(ROOT/'.local/bin/jellyfish-prefix')
 cmd+=[str(fa)];t=time.monotonic();subprocess.run(cmd,check=True,env=dict(os.environ,SAT_ASO_PREFIX_FILTER='1'));elapsed=time.monotonic()-t
 ...
 print('prefix',use,'seconds',elapsed,'nonzero_kmers',len(counts),flush=True)
 return counts
