...
このワークフローは既存のSTEP 0–4とtop 3 / 最大72配列のCHM13位置解析を、依存関係を持つジョブとして実行します。閾値sweepジョブはありません。長時間処理の前に`preflight`が全ユニットテストとSTEP間の表schema契約を検査し、廃止した列名や出力・読込定義の不一致を早期検出します。結果は `runs/<run_id>/` に作成します。ルートの既存 `step0/`〜`step4/` は変更しません。実行時には現在の`scripts/`をrun directoryにコピーし、そのSHA256と設定ファイルのSHA256を`.bootstrap.json`に記録します。同じ`run_id`で設定やコードが変わった場合、混在を防ぐため停止します。新しい`run_id`を使ってください。
